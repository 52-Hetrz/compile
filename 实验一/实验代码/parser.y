...
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN FOR SWITCH CASE COLON DEFAULT BREAK CONTINUE ARRAY ARRAYSIZE FIRSTDPLUS LASTDPLUS FIRSTDMINUS LASTDMINUS 
...
ExtDef:   Specifier ExtDecList SEMI   {$$=mknode(2,EXT_VAR_DEF,yylineno,$1,$2);}   //该结点对应一个外部变量声明
          ...
          | error SEMI   {$$=NULL;}
          ;
Specifier:  TYPE    {$$=mknode(0,TYPE,yylineno);strcpy($$->type_id,$1);$$->type=!strcmp($1,"int")?INT:FLOAT;}   		//函数对应的返回值
           ;      
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | CHAR	      {$$=mknode(0,CHAR,yylineno);strcpy($$->type_char,$1);}
