...
%token EXT_DEF_LIST EXT_VAR_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE EXP_ARRAY
%token FUNC_CALL ARGS FUNCTION PARAM ARG CALL LABEL GOTO JLT JLE JGT JGE EQ NEQ


%right MINUSASSIGNOP PLUSASSIGNOP DIVASSIGNOP STARASSIGNOP
%left ASSIGNOP
%left OR
%left AND
%left RELOP
...
program: ExtDefList    { printf("程序开始： \n");display($1,0);printf("\n\n中间代码：\n\n");DisplaySymbolTable($1);}     //显示语法树,语义分析
         ; 
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
      ...
      | Exp PLUS ASSIGNOP Exp {$$=mknode(2,PLUSASSIGNOP,$1,$4,yylineno);strcpy($$->type_id,"PLUSASSIGNOP");}
      | Exp MINUS ASSIGNOP Exp {$$=mknode(2,MINUSASSIGNOP,$1,$4,yylineno);strcpy($$->type_id,"MINUSASSIGNOP");}
      | Exp STAR ASSIGNOP Exp {$$=mknode(2,STARASSIGNOP,$1,$4,yylineno);strcpy($$->type_id,"STARASSIGNOP");}
      | Exp DIV ASSIGNOP Exp {$$=mknode(2,DIVASSIGNOP,$1,$4,yylineno);strcpy($$->type_id,"DIVASSIGNOP");}
